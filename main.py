...
def load_data():
    try:
        with open("debate_data.json") as f:
            return json.load(f)
    except Exception:
        return {
            "users": {},
...
def save_data(data):
    with open("debate_data.json", "w") as f:
...
def edit_website():
    ...
    data = load_data()
    if request.method == 'POST':
        data['site_content']['title'] = request.form.get('title', '').strip()
        data['site_content']['subtitle'] = request.form.get('subtitle', '').strip()
        save_data(data)
...
def forum():
    data = load_data()
    if not data['settings'].get('forum_enabled', True):
        flash('Forum is currently disabled.', 'error')
        return redirect(url_for('index'))
    posts = sorted(data['forum_posts'], key=lambda x: x['date'], reverse=True)
    return render_template('forum.html', posts=posts, data=data)

@app.route('/moderate_forum', methods=['GET'])
def moderate_forum():
    if not requires_admin():
        flash('Admin access required!', 'error')
        return redirect(url_for('login'))
    data = load_data()
    posts = sorted(data['forum_posts'], key=lambda x: x['date'], reverse=True)
    return render_template('moderate_forum.html', posts=posts, data=data)
...
@app.route('/my_rubrics')
def my_rubrics():
    if not requires_login():
        flash('Login required to view your rubrics.', 'error')
        return redirect(url_for('login'))
    ...
    my_rubrics = {
        rid: rubric for rid, rubric in data.get('rubrics', {}).items()
        if rubric.get('created_by') == user['username']
    }
